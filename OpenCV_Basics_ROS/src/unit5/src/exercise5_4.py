...
class LoadPolygon(object):
    ...
    def camera_callback(self,data):
        ...
        try:
            ...
            # Order it so a rectangle can be drawn
            if len(params) >= 4:
                _params = np.zeros(params.shape, dtype="int")
                
                s = np.sum(params, axis = 1)
                _params[0] = params[np.argmin(s)]
                _params[2] = params[np.argmax(s)]
                
                d = np.diff(params, axis = 1)
                _params[1] = params[np.argmin(d)]
                _params[3] = params[np.argmax(d)]
